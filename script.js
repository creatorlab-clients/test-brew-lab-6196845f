/* ==========================================================
   Brew Lab — script.js
   Scroll animation (food-modern 211 frame, cover §4.2)
...
(function () {
  ...
  // Scroll Animation — food-modern (211 frame, cover mode §4.2)
  ...
  if (canvas && section) {
    ...
    var FRAME_PATH = 'https://8ispuxmgjxgu2r5q.public.blob.vercel-storage.com/templates/bar-002/frames/';
    var FRAME_COUNT = 211;
  ...
  if ('IntersectionObserver' in window) {
    ...
    var stagObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) {
          var d = parseInt(e.target.dataset.stagger || 0, 10) * 160;
...
window.__imgFallback = function (img, label) {
  ...
  w.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice">' +
    ...
    '<text x="400" y="320" text-anchor="middle" font-family="sans-serif" font-size="24" fill="currentColor" opacity="0.4">' + label + '</text></svg>';
  img.replaceWith(w);
};
